...
class PPO:
    ...
    def compute_gae(self, rewards, values, dones):
        """Generalized Advantage Estimation; lambda trades off bias vs variance."""
        T = len(rewards)

        advantages = torch.zeros(T, device=self.device)
        gae = torch.zeros((), device=self.device)

        # walk backwards so each step can bootstrap from future ones
    ...
    def update_networks(
        ...
        for _ in range(epoch):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                # clipped surrogate: ratio bounded to [1-e, 1+e] prevents too-large steps
                ratio = (new_log_probs - mb_old_log_probs).exp()
                actor_loss = -torch.minimum(ratio * mb_advantages, torch.clamp(ratio, 1-self.e, 1+self.e) * mb_advantages).mean()
                actor_loss = actor_loss - 0.01 * entropy  # entropy bonus to avoid early collapse
